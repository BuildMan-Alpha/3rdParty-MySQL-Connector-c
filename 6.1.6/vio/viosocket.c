...
*/
static void vio_get_normalized_ip(Vio* vio,
                                  ...
                                  size_t *dst_length)
{
  ...
  case AF_INET6:
  {
  if (vio->mysql_socket.allow_ipv6)
	  {
...
my_bool vio_get_normalized_ip_string(Vio* vio, const struct sockaddr *addr,
                                     size_t addr_length,
                                     char *ip_string,
                                     size_t ip_string_size)
{
  ...
  vio_get_normalized_ip(vio, addr, addr_length, norm_addr, &norm_addr_length);

  err_code= vio_getnameinfo(vio, norm_addr, ip_string, ip_string_size, NULL, 0, NI_NUMERICHOST);
...
my_bool vio_peer_addr(Vio *vio, char *ip_buffer, uint16 *port, size_t ip_buffer_size)
{
  ...
  else
  {
    ...
    vio_get_normalized_ip(vio, addr, addr_length,
                          (struct sockaddr *) &vio->remote, &vio->addrLen);

    /* Get IP address & port number. */

    err_code= vio_getnameinfo(vio, (struct sockaddr *) &vio->remote,
...
int vio_getnameinfo(Vio* vio, const struct sockaddr *sa,
