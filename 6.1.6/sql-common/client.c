		       ...
		       uint port, const char *unix_socket,ulong client_flag)
{
  ...
  */
       ...
       mysql->options.protocol == MYSQL_PROTOCOL_TCP))
  {
    ...
    hints.ai_family= (mysql->options.allow_ipv6 ? AF_UNSPEC : AF_INET);
    ...
    */
    ...
    for (t_res= res_lst; t_res; t_res= t_res->ai_next)
    {
	  ...
	  net->vio->mysql_socket.allow_ipv6 = mysql->options.allow_ipv6;
...
mysql_options(MYSQL *mysql,enum mysql_option option, const void *arg)
{
  ...
  case MYSQL_OPT_WRITE_TIMEOUT:
    ...
    break;
  case MYSQL_OPT_ALLOW_IPV6:
	  mysql->options.allow_ipv6 = *(my_bool*) arg;
...
mysql_get_option(MYSQL *mysql, enum mysql_option option, const void *arg)
{
  ...
  case MYSQL_OPT_PROTOCOL:
    *((uint *)arg)= mysql->options.protocol;
    break;
  case MYSQL_OPT_ALLOW_IPV6:
	  *((my_bool*)arg) = mysql->options.allow_ipv6;
	  break;
  case MYSQL_SHARED_MEMORY_BASE_NAME:
#if defined (_WIN32) && !defined (EMBEDDED_LIBRARY)
